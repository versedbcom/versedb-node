...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetVariantDetailsRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/api/v1/issues/{issue_id}/variants/{variant_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetVariantDetailsResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
