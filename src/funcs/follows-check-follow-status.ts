 ...
 * Check follow status.
 */
export function followsCheckFollowStatus(
  client: VerseDBCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.CheckFollowStatusRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/api/v1/follow/{type}/{id}/check")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CheckFollowStatusResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
