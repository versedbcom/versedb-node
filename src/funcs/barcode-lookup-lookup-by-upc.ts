 ...
 */
export function barcodeLookupLookupByUPC(
  client: VerseDBCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.LookupByUPCRequest$outboundSchema, value),
  ...
  const pathParams = {
    upc: encodeSimple("upc", payload.upc, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/lookup/upc/{upc}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.LookupByUPCResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
