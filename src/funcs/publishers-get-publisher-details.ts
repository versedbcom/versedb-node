 ...
 */
export function publishersGetPublisherDetails(
  client: VerseDBCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetPublisherDetailsRequest$outboundSchema, value),
  ...
  const pathParams = {
    publisher_id: encodeSimple("publisher_id", payload.publisher_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/publishers/{publisher_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetPublisherDetailsResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
