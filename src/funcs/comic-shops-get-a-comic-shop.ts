...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetAComicShopRequest$outboundSchema, value),
  ...
  const pathParams = {
    shop_id: encodeSimple("shop_id", payload.shop_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/shops/{shop_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAComicShopResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
