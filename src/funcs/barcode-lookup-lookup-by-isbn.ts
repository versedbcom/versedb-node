 ...
 */
export function barcodeLookupLookupByISBN(
  client: VerseDBCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.LookupByISBNRequest$outboundSchema, value),
  ...
  const pathParams = {
    isbn: encodeSimple("isbn", payload.isbn, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/lookup/isbn/{isbn}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.LookupByISBNResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
