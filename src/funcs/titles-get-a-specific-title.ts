 ...
 */
export function titlesGetASpecificTitle(
  client: VerseDBCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetASpecificTitleRequest$outboundSchema, value),
  ...
  const pathParams = {
    title_id: encodeSimple("title_id", payload.title_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/titles/{title_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetASpecificTitleResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
