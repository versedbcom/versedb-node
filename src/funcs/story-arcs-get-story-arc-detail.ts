 ...
 */
export function storyArcsGetStoryArcDetail(
  client: VerseDBCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetStoryArcDetailRequest$outboundSchema, value),
  ...
  const pathParams = {
    storyArc_id: encodeSimple("storyArc_id", payload.storyArc_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/story-arcs/{storyArc_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetStoryArcDetailResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
