...
import { VerseDbDefaultError } from "../models/errors/verse-db-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | VerseDbDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | VerseDbDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new VerseDbDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new VerseDbDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
